      ...
      {items.map((p) => {
        ...
        return (
          <section key={p.id} data-post className="h-screen snap-start relative flex items-end justify-center"
            style={{background:'radial-gradient(1200px 600px at 50% 0%, rgba(6,182,212,0.08), transparent), radial-gradient(900px 400px at 50% 100%, rgba(217,70,239,0.10), transparent)'}}>
            <div className="absolute left-4 bottom-32 md:bottom-10 space-y-2 pointer-events-none max-w-[60%]">
              ...
              {p.bpm && (
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/20 border border-purple-500/30 text-purple-400 text-xs font-medium">
                  ⚡ {p.bpm} BPM
                </div>
              )}
            </div>
